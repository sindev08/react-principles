...
type StateRow = { boxCls: string; label: string; stateLabel: string; showCheck?: boolean; showMinus?: boolean };

const FORCED_STATES: Record<"light" | "dark", StateRow[]> = {
  light: [
...
function ThemedCheckboxPreview({ theme }: { theme: "light" | "dark" }) {
  ...
  return (
    <div className={`rounded-xl border ${border} ${bg} p-6 space-y-4`}>
      {FORCED_STATES[theme].map((s) => (
        <div key={s.stateLabel} className="flex items-center gap-3">
          <div className={`h-5 w-5 shrink-0 rounded flex items-center justify-center ${s.boxCls}`}>
            {s.showCheck && (
    ...
    <DocsPageLayout tocItems={TOC_ITEMS}>
      <div className="max-w-4xl">
        ...
        <section id="snippet" className="mb-16">
          ...
          <CodeBlock filename="components/ui/Checkbox.tsx" copyText={CODE_SNIPPET}>
            {CODE_SNIPPET}
          </CodeBlock>
        </section>
